...
function BulkScanStatus({ progress }: { progress: BulkScanProgress }) {
  ...
  if (progress.status === "scanning") {
    ...
    return (
      <div
        className="flex items-center gap-2 px-2.5 py-1 text-xs text-text-secondary"
      ...
      >
        ...
        <span className="whitespace-nowrap">
          {progress.current}/{progress.total}: {progress.currentKit}
        </span>
      </div>
    );
  }
...
const KitBrowserHeader: React.FC<KitBrowserHeaderProps> = (props) => {
  return (
    <div
      className="sticky top-0 z-10 bg-surface-2 px-3 py-2 flex flex-col border-b border-border-subtle"

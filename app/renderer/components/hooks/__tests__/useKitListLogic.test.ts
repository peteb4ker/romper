...
describe("useKitListLogic", () => {
  it("sorts kits and provides helpers", () => {
    ...
    expect(result.current.showBankAnchor("A1", 0, ["A1", "A2", "B2"])).toBe(
      true,
    );
    expect(result.current.showBankAnchor("A2", 1, ["A1", "A2", "B2"])).toBe(
      false,
    );
    expect(result.current.showBankAnchor("B2", 2, ["A1", "A2", "B2"])).toBe(
      true,
    );
  });
});

...
import { createDefaultStepPattern, ensureValidStepPattern } from "./stepPatternConstants";
 ...
 */
export function useStepPattern({ kitName, initialPattern }: UseStepPatternParams) {
  ...
  const updateStepPattern = useCallback(async (pattern: number[][]) => {
    ...
    try {
      const result = await window.electronAPI.updateStepPattern(kitName, pattern);
      if (!result.success) {

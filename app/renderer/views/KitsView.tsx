...
import { compareKitSlots, groupSamplesByVoice } from "@romper/shared";
...
const KitsView = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // 3. Load labels
      const loadedLabels: RampleLabels | null = await window.electronAPI
      ...
      {selectedKit && selectedKitSamples ? (
        <KitDetails
          ...
          onRequestSamplesReload={async () => {
            ...
            const files: string[] = await window.electronAPI
              ?.listFilesInRoot?.(kitPath)
              .catch(() => []);
          ...
          onBack={async (scrollToKitOrObj) => {
            ...
            let refresh = false;
            if (
              typeof scrollToKitOrObj === "object" &&
              scrollToKitOrObj !== null
            ) {
            ...
            if (scrollToKit) {
              setTimeout(() => {
                ...
                const kitEl = document.querySelector(
                  `[data-kit='${scrollToKit}']`,
                );

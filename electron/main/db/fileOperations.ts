...
export async function deleteDbFileWithRetry(
  ...
  console.log(
    `[DELETE] deleteDbFileWithRetry: Starting deletion of ${dbPath} (Windows: ${isWindows}, maxRetries: ${maxRetries})`,
  );

  for (let i = 0; i < maxRetries; i++) {
    const attemptStart = Date.now();
    console.log(
      `[DELETE] deleteDbFileWithRetry: Attempt ${i + 1}/${maxRetries} for ${dbPath}`,
    );
    ...
    try {
      ...
      if (success) {
        ...
        console.log(
          `[DELETE] deleteDbFileWithRetry: Successfully deleted ${dbPath} on attempt ${i + 1} (attempt: ${attemptDuration}ms, total: ${totalDuration}ms)`,
        );
        return;
      } else {
        console.log(
          `[DELETE] deleteDbFileWithRetry: Attempt ${i + 1} unsuccessful after ${attemptDuration}ms, continuing...`,
        );
      }
    } catch (error) {
      ...
      const attemptDuration = Date.now() - attemptStart;
      console.log(
        `[DELETE] deleteDbFileWithRetry: Attempt ${i + 1} failed after ${attemptDuration}ms:`,
  ...
  if (isWindows) {
    console.log(
      `[DELETE] deleteDbFileWithRetry: All ${maxRetries} attempts failed, trying final Windows rename fallback`,
    );
    ...
    try {
      ...
      const totalDuration = Date.now() - startTime;
      console.log(
        `[DELETE] deleteDbFileWithRetry: Final Windows rename completed in ${fallbackDuration}ms (total: ${totalDuration}ms)`,
      );
      return; // Always return on Windows after final attempt
    } catch (error) {
      const fallbackDuration = Date.now() - fallbackStart;
      console.error(
        `[DELETE] deleteDbFileWithRetry: Final Windows rename failed after ${fallbackDuration}ms:`,
        error,
      );
    }
  }
  ...
  const totalDuration = Date.now() - startTime;
  console.error(
    `[DELETE] deleteDbFileWithRetry: Failed to delete ${dbPath} after ${totalDuration}ms and ${maxRetries} attempts`,
  );
...
async function handleDeleteAttempt(
  ...
  console.log(
    `[DELETE] handleDeleteAttempt: Starting attempt ${attempt + 1} for ${dbPath} (Windows: ${isWindows}, lastAttempt: ${isLastAttempt})`,
  );
  ...
  try {
    ...
    if (isWindows && attempt > 0) {
      console.log(
        `[DELETE] handleDeleteAttempt: Windows platform, waiting before retry attempt ${attempt + 1}`,
      );
    ...
    console.log(
      `[DELETE] handleDeleteAttempt: Attempting actual file deletion for ${dbPath}`,
    );
    const deleteResult = await tryDelete(dbPath, attempt);
    console.log(
      `[DELETE] handleDeleteAttempt: Delete attempt ${attempt + 1} result: ${deleteResult}`,
    );
    return deleteResult;
  } catch (error) {
    const err = error as Error;
    console.log(
      `[DELETE] handleDeleteAttempt: Delete attempt ${attempt + 1} failed:`,
      err.message,
    );
    ...
    if (isWindows && !isLastAttempt) {
      console.log(
        `[DELETE] handleDeleteAttempt: Windows platform, trying rename fallback for attempt ${attempt + 1}`,
      );
      const fallbackResult = await tryWindowsRenameFallback(dbPath, attempt);
      console.log(
        `[DELETE] handleDeleteAttempt: Windows rename fallback result: ${fallbackResult}`,
      );
    ...
    if (!isWindows) {
      console.log(
        `[DELETE] handleDeleteAttempt: Non-Windows platform, waiting before next attempt`,
      );
      await waitForRetry(attempt, isWindows);
    }
...
async function waitForRetry(
  ...
  if (delay > 0) {
    console.log(
      `[DELETE] waitForRetry: Waiting ${delay}ms before retry attempt ${attempt + 1} (Windows: ${isWindows}, renameFailure: ${isRenameFailure})`,
    );
    const waitStart = Date.now();
    await new Promise((resolve) => setTimeout(resolve, delay));
    const waitDuration = Date.now() - waitStart;
    console.log(
      `[DELETE] waitForRetry: Wait completed after ${waitDuration}ms`,
    );
  } else {
    console.log(
      `[DELETE] waitForRetry: No delay needed for attempt ${attempt + 1}`,
    );
  }
}

...
describe("SampleService", () => {
  ...
  describe("addSampleToSlot", () => {
    it("successfully adds valid sample", () => {
      ...
      expect(mockMarkKitAsModified).toHaveBeenCalledWith("/test/path/.romperdb", "TestKit");
    });
  ...
  describe("validateSampleSources", () => {
    it("validates all samples for a kit", () => {
      ...
      const result = sampleService.validateSampleSources(mockInMemorySettings, "TestKit");
    ...
    it("identifies invalid sample sources", () => {
      ...
      expect(result.data?.invalidSamples[0].error).toBe("Sample file not found");
    });

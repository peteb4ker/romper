...
describe('main/index.ts', () => {
  ...
  it('falls back to empty settings if file is invalid JSON', async () => {
    const readFileSyncSpy = vi.spyOn(fs, 'readFileSync').mockReturnValue('not json');
    await import('../index');
  ...
  it('does not throw if unhandledRejection is triggered with no error', async () => {
    const spy = vi.spyOn(console, 'error').mockImplementation(() => {});
    await import('../index');
    expect(() => process.emit('unhandledRejection', undefined, Promise.resolve())).not.toThrow();
    spy.mockRestore();
  });
  ...
  it('logs error if settings file is invalid JSON', async () => {
    ...
    expect(spy).toHaveBeenCalledWith(
      expect.stringContaining('Failed to parse settings file. Using empty settings:'),
      expect.any(Error)
    );
    spy.mockRestore();
    readFileSyncSpy.mockRestore();
  ...
  it('logs info if settings file is loaded', async () => {
    const readFileSyncSpy = vi.spyOn(fs, 'readFileSync').mockReturnValue('{"foo": "bar"}');

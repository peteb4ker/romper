...
export function registerIpcHandlers(watchers: { [key: string]: fs.FSWatcher }, inMemorySettings: Record<string, any>) {
    ...
    ipcMain.handle('scan-sd-card', async (event, sdCardPath) => {
        return fs.readdirSync(sdCardPath).filter((folder) => /^[A-Z][0-9]{1,2}$/.test(folder));
    });
    ipcMain.handle('select-sd-card', async () => {
        const result = await dialog.showOpenDialog({ properties: ['openDirectory'], title: 'Select SD Card Path' });
    ...
    ipcMain.handle('watch-sd-card', (event, sdCardPath: string) => {
        ...
        const watcher = fs.watch(sdCardPath, { persistent: true }, (eventType, filename) => {
            if (filename) event.sender.send('sd-card-changed', { eventType, filename });
        });
    ...
    ipcMain.handle('create-kit', async (_event, sdCardPath: string, kitSlot: string) => {
        if (!/^[A-Z][0-9]{1,2}$/.test(kitSlot)) throw new Error('Invalid kit slot. Use format A0-Z99.');
        ...
        labels.kits[kitSlot] = labels.kits[kitSlot] || { label: kitSlot, plan: [] };
    ...
    ipcMain.handle('copy-kit', async (_event, sdCardPath: string, sourceKit: string, destKit: string) => {
        if (!/^[A-Z][0-9]{1,2}$/.test(sourceKit) || !/^[A-Z][0-9]{1,2}$/.test(destKit)) throw new Error('Invalid kit slot. Use format A0-Z99.');
        const srcPath = path.join(sdCardPath, sourceKit);
        const destPath = path.join(sdCardPath, destKit);
        if (!fs.existsSync(srcPath)) throw new Error('Source kit does not exist.');
        if (fs.existsSync(destPath)) throw new Error('Destination kit already exists.');
        copyRecursiveSync(srcPath, destPath);
    });
    ipcMain.handle('list-files-in-root', async (_event, sdCardPath: string) => fs.readdirSync(sdCardPath));
    ipcMain.handle('get-audio-buffer', async (_event, filePath: string) => {
        const data = fs.readFileSync(filePath);
        return data.buffer.slice(data.byteOffset, data.byteOffset + data.byteLength);
    });
    ipcMain.handle('read-rample-bin-all', async (_event, filePath: string) => parseRampleBin(filePath));
    ipcMain.handle('read-rample-labels', async (_event, sdCardPath: string) => readRampleLabels(sdCardPath));
    ...
    ipcMain.handle('discard-kit-plan', async (_event, sdCardPath: string, kitName: string) => {
        const errors: string[] = [];
        try {
            ...
            try { writeRampleLabels(sdCardPath, labels); } catch (e) {
                errors.push(`Failed to update labels file: ${e instanceof Error ? e.message : String(e)}`);
                return { success: false, errors };
            }
            return { success: true };
        } catch (e) {
    ...
    ipcMain.handle('rescan-all-voice-names', async (_event, sdCardPath: string, kitNames: string[]) => {
        const labels = readRampleLabels(sdCardPath) || { kits: {} };
        for (const kitName of kitNames) {
            const kitPath = path.join(sdCardPath, kitName);
            const wavFiles = fs.existsSync(kitPath) ? fs.readdirSync(kitPath).filter(f => /\.wav$/i.test(f)) : [];
            const voices = groupSamplesByVoice(wavFiles);
            const newVoiceNames: { [key: number]: string } = { 1: '', 2: '', 3: '', 4: '' };

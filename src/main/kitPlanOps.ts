...
export function validateKitPlan(plan: any[]): string[] {
    ...
    if (duplicateSource) {
        errors.push(`Sample '${duplicateSource}' is assigned more than once in the plan. Each sample file can only be used once in the plan.`);
    }
    for (const [src, voices] of Object.entries(sourceToVoices)) {
        if (voices.size > 1) {
            errors.push(`Sample '${src}' is assigned to multiple voices (${[...voices].join(', ')}). Each sample file can only be used in one voice slot.`);
        }
    }
    return errors;
}
...
export function writeKitSamples(plan: any[], kitPath: string) {
    if (fs.existsSync(kitPath)) {
        ...
        for (const file of files) {
            if (/\.wav$/i.test(file)) {
                try { fs.unlinkSync(path.join(kitPath, file)); } catch (e) { /* ignore for test */ }
            }
        }
    }
    ...
    for (let voice = 1; voice <= 4; voice++) {
        ...
        for (let i = 0; i < slots.length; i++) {
            ...
            newTarget = newTarget.replace(/^([1-4])(\.| )/, `${voice}$2`).replace(/^([1-4])(?=[A-Z])/, `${voice}`);
...
export async function commitKitPlanHandler(sdCardPath: string, kitName: string) {
    const errors: string[] = [];
    ...
    if (!fs.existsSync(kitPath)) {
        try { fs.mkdirSync(kitPath); } catch (e) { errors.push(`Failed to create kit folder: ${e instanceof Error ? e.message : String(e)}`); }
    }

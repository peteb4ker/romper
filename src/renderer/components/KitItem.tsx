...
// Helper to determine which icon to show based on voice labels
function getKitIcon(voiceNames?: Record<string|number, string>): JSX.Element {
    // Always treat as object, get values
    const arr = voiceNames ? Object.values(voiceNames).filter(v => typeof v === 'string' && v.trim() && v !== '...' && v !== 'loading') : [];
    if (!arr.length) {
        return <FiFolder className="text-3xl" />;
    }
    const unique = Array.from(new Set(arr.map(v => v.toLowerCase())));
    if (unique.length === 1) {
        if (unique[0] === 'vox') return <MdMic className="text-3xl text-pink-600 dark:text-pink-300" />;
        if (unique[0] === 'loop') return <TiArrowLoop className="text-3xl text-amber-600 dark:text-amber-300" />;
        if (unique[0] === 'fx') return <MdAutoAwesome className="text-3xl text-indigo-600 dark:text-indigo-300" />;
        if (unique[0] === 'synth' || unique[0] === 'bass') return <BiSolidPiano className="text-3xl text-blue-700 dark:text-blue-300" />;
    }
    if (unique.length > 1 && unique.every(v => v === 'synth' || v === 'bass')) {
        return <BiSolidPiano className="text-3xl text-blue-700 dark:text-blue-300" />;
    }
    return <GiDrumKit className="text-3xl text-yellow-700 dark:text-yellow-300" />;
}
    ...
    >
        ...
        <div className="flex flex-row w-full h-full items-stretch">
            ...
            <div className="flex items-center justify-center w-auto min-w-0 p-0 m-0 pr-2">
                <span className="block text-5xl" style={{ lineHeight: 1 }}>
                    {getKitIcon(kitLabel?.voiceNames)}
                </span>
            </div>
            {/* Right column: 2 rows */}
            <div className="flex flex-col flex-1 min-w-0">
                ...
                {isValid && kitLabel?.voiceNames && (
                    <div className="flex flex-wrap gap-1 mt-1 w-full min-h-[1.5rem] justify-end">
                        {Object.values(kitLabel.voiceNames).filter(Boolean).map((label, i) => (
                            <span key={i} className="bg-blue-100 dark:bg-blue-800 text-blue-800 dark:text-blue-100 px-1 py-0.5 rounded text-[10px] font-mono rounded-sm">

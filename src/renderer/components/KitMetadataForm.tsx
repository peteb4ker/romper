...
const KitMetadataForm: React.FC<KitMetadataFormProps> = ({ kitLabel, loading, error, editing = false, onEdit, onCancel, onSave, hideDescription, tagsEditable }) => {
  ...
  if (loading) return <div className="text-xs text-gray-400">Loading kit metadata...</div>;
      ...
      {tagsEditable && (
        <div className="flex flex-wrap gap-1 items-center">
          {editingTags ? (
            <>
              {tags.map((tag, idx) => (
                <span key={tag} className="bg-blue-200 dark:bg-blue-700 text-blue-900 dark:text-blue-100 px-2 py-0.5 rounded-full text-xs flex items-center gap-1">
                  {tag}
                  <button
              ...
              <button
                ...
                onClick={() => { setEditingTags(false); onSave(kitLabel?.label || '', '', tags); }}
                type="button"
              >
                Save
              </button>
              <button
                ...
                onClick={() => { setEditingTags(false); setTags(kitLabel?.tags || []); setTagInput(''); }}
                type="button"
              >
                Cancel
              </button>

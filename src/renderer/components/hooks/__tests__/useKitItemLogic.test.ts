...
import { useKitItemLogic } from '../useKitItemLogic';
import { renderHook } from '@testing-library/react';

describe('useKitItemLogic', () => {
  it('returns folder iconType and label if no voiceNames', () => {
    const { result } = renderHook(() => useKitItemLogic(undefined));
  ...
  it('returns correct iconType/label for vocal kit', () => {
    const { result } = renderHook(() => useKitItemLogic({ 1: 'vox', 2: 'vox', 3: 'vox', 4: 'vox' }));
  ...
  it('returns correct iconType/label for loop kit', () => {
    const { result } = renderHook(() => useKitItemLogic({ 1: 'loop', 2: 'loop', 3: 'loop', 4: 'loop' }));
  ...
  it('returns correct iconType/label for fx kit', () => {
    const { result } = renderHook(() => useKitItemLogic({ 1: 'fx', 2: 'fx', 3: 'fx', 4: 'fx' }));
  ...
  it('returns correct iconType/label for synth/bass kit', () => {
    const { result } = renderHook(() => useKitItemLogic({ 1: 'synth', 2: 'bass', 3: 'synth', 4: 'bass' }));
  ...
  it('returns drumkit iconType/label for mixed or other', () => {
    const { result } = renderHook(() => useKitItemLogic({ 1: 'kick', 2: 'snare', 3: 'hat', 4: 'perc' }));

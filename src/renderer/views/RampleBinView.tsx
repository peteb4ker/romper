...
const formatHex = (arr: number[] | Buffer) =>
  Array.from(arr)
...
const RampleBinView: React.FC<RampleBinViewProps> = ({ sdCardPath }) => {
  ...
  useEffect(() => {
    ...
    // @ts-ignore
    window.electronAPI
      .readRampleBinAll?.(`${sdCardPath}/rample.bin`)
      .then(setData)
      .catch((e) =>
        setError(
          e.message.includes("no such file")
            ? "rample.bin not found on SD card."
            : e.message,
        ),
      );
  }, [sdCardPath]);
      ...
      <table className="table-auto border-collapse w-full text-xs">
        <tbody>
          <tr className="border-b">
            <th className="text-left pr-2">Header</th>
            <td className="font-mono">
              Magic: {data.header.magic}, Version: {data.header.version},
              Reserved: {data.header.reserved.join(" ")}
            </td>
          </tr>
          <tr className="border-b">
            <th className="text-left pr-2 align-top">Kit Assignments</th>
            <td>
          ...
          <tr className="border-b">
            <th className="text-left pr-2">Global Settings</th>
            <td className="font-mono">
              {data.globalSettings
                .map((b) => b.toString(16).padStart(2, "0"))
                .join(" ")}
          ...
          <tr>
            <th className="text-left pr-2">File Size</th>
...
const RampleLabelsSection: React.FC<{ sdCardPath: string | null }> = ({
  sdCardPath,
}) => {
                ...
                <tr key={kit}>
                  ...
                  <td className="px-2">
                    <input
                      ...
                      value={meta.label}
                      onChange={(e) =>
                        handleFieldChange(kit, "label", e.target.value)
                      }
                    />
                  </td>
                  <td className="px-2">
                    <input
                      className="border rounded px-1 w-48"
                      value={meta.description || ""}
                      onChange={(e) =>
                        handleFieldChange(kit, "description", e.target.value)
                      }
                    />
                  </td>
                  <td className="px-2">
